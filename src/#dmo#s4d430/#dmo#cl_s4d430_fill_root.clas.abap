CLASS /DMO/cl_s4d430_fill_root DEFINITION
  PUBLIC
  ABSTRACT.
...
CLASS /DMO/CL_S4D430_FILL_ROOT IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    APPEND NEW lcl_table( i_name = i_employ_table
                          i_source =  SWITCH #( i_version
                                           WHEN employee_table_only      THEN '/DMO/EMPLOY'
                                           WHEN with_relationships    THEN '/DMO/EMPLOY_REL'
                                           WHEN with_extensions THEN '/DMO/EMPLOY_EXT')
                         )
        TO tables.

    IF i_version = with_relationships.
      APPEND NEW lcl_table( i_name = i_employ_table
                            i_source = '/DMO/DEPMENT_REL'
                           )
          TO tables.
    ENDIF.
  ENDMETHOD.
